...
export default class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      const msg = this.state.error?.message || 'Tidak ada detail error';
      return (
        <div className="min-h-screen bg-light-bg dark:bg-dark-bg flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white dark:bg-dark-card rounded-2xl border border-gray-100 dark:border-dark-border shadow-sm p-8 text-center">
            ...
            <div className="mb-6 px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-xl text-left">
              <p className="text-xs font-mono text-red-600 dark:text-red-400 break-all">{msg}</p>
            </div>
